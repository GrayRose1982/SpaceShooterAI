...
public class BluetoothMultiplayerAndroidDemoGui : MonoBehaviour
{
	...
	// reference to out test actor

	private const string LocalIp = "127.0.0.1";
	// An IP for Network.Connect(), must always be 127.0.0.1
	private const int Port = 28000;
	// Local server IP. Must be the same for client and server
	...
	void Update ()
	{
		...
		if (Input.GetKeyDown (KeyCode.Escape)) {
			...
			SceneManager.LoadScene ("BluetoothMultiplayerAndroidDemoMenu");
		}
	}
